...
import { getDictionary, getScopedTranslator } from '@/lib/i18n/dictionaries';
...
export async function generateMetadata({params}: {params: Promise<{locale: string}>}) {
  ...
  try {
    const t = getScopedTranslator(locale, 'Layout');
    return {
      title: t('title'),
          ...
          <div className="flex flex-col min-h-screen">
            <header className="glass-panel sticky top-0 z-50 rounded-none border-t-0 border-x-0 border-b border-white/10 px-6 py-4 flex items-center justify-between">
              ...
              <nav className="hidden md:flex flex-1 justify-center space-x-3 lg:space-x-4 text-xs lg:text-sm px-4 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <Link href={`/${locale}`} className="flex items-center space-x-1 hover:text-blue-400 transition-colors whitespace-nowrap">
                  <Trophy className="w-4 h-4 hidden xl:block" />
                  <span>{t('dashboard')}</span>
                </Link>
                <Link href={`/${locale}/schedule`} className="flex items-center space-x-1 hover:text-blue-400 transition-colors whitespace-nowrap">
                  <CalendarDays className="w-4 h-4 hidden xl:block" />
                  <span>{t('schedule')}</span>
                </Link>
                <Link href={`/${locale}/news`} className="flex items-center space-x-1 hover:text-orange-400 transition-colors whitespace-nowrap">
                  <Newspaper className="w-4 h-4 hidden xl:block" />
                  <span>{t('news')}</span>
                </Link>
                <Link href={`/${locale}/true-standings`} className="flex items-center space-x-1 hover:text-emerald-400 transition-colors whitespace-nowrap">
                  <Activity className="w-4 h-4 hidden xl:block" />
                  <span>{t('trueStandings')}</span>
                </Link>
                <Link href={`/${locale}/multiverse`} className="flex items-center space-x-1 hover:text-purple-400 transition-colors whitespace-nowrap">
                  <ArrowRightLeft className="w-4 h-4 hidden xl:block" />
                  <span>{t('multiverse')}</span>
                </Link>
                <Link href={`/${locale}/rivalries`} className="flex items-center space-x-1 hover:text-red-400 transition-colors whitespace-nowrap">
                  <Swords className="w-4 h-4 hidden xl:block" />
                  <span>{t('rivalries')}</span>
                </Link>
                <Link href={`/${locale}/draft-report`} className="flex items-center space-x-1 hover:text-yellow-400 transition-colors whitespace-nowrap">
                  <Search className="w-4 h-4 hidden xl:block" />
                  <span>{t('draftReport')}</span>
                </Link>
                <Link href={`/${locale}/history`} className="flex items-center space-x-1 hover:text-cyan-400 transition-colors whitespace-nowrap">
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center space-x-2 bg-slate-800/50 rounded-lg p-1 border border-white/5">
                  <a href="/en" className={`px-2 py-1 rounded text-xs font-bold transition-colors ${locale === "en" ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-white"}`}>EN</a>
                  <a href="/uk" className={`px-2 py-1 rounded text-xs font-bold transition-colors ${locale === "uk" ? "bg-indigo-500 text-white" : "text-slate-400 hover:text-white"}`}>UK</a>
                </div>
                <RefreshButton />
              </div>
            </header>
